...
setPlayAgain(true)
  ...
  return (
    <div><div style={{color:"white"}}>Hex Code : {code}</div>
        
        {colorcode.map((e)=><div className="colors" style={{background:e}} key={e} onClick={()=>match(e)}></div>)}
        {message? <p style={{color:"red",fontSize:"20px"}}>{message}</p>: " "}
        <div>{playAgain ? <button onClick={()=>window.location.reload(true)}>PlayAgain</button> : " "}</div>
    </div>
  )
}
